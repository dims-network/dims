...
#
# The last one. rqa.py and crqa.py now take their parameters as parameters --
# see the analyse()/main()/run() split there, and the cross-project read it
# closed -- and this file should follow. It has not, because it is the harder
# half: main() is 200 lines, and the worker-state machinery below
# (_WORKER_STATE, _worker_state, _apply_worker_state) exists precisely because
# the analysis reads module globals across a `spawn` boundary. Parameterising
# the step is what would let those go too, and that is a day's work with a
# Monte Carlo and a 1e-6 baseline underneath it, not an hour's.
#
# It is safe as it stands: the CLI builds one StepContext per invocation, so
# nothing runs two projects in one process. What makes it worth doing is that
# `os.chdir` is process-global, which makes this step unusable from a threaded
# caller -- while it spawns a ProcessPoolExecutor underneath itself.
#
# NOT tracked as an issue. The comment used to say it was; it was not, in any
# issue, changelog entry or document in this repository.
# ---------------------------------------------------------------------------
